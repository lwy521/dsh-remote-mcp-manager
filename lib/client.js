window.__ModuleLoader__.load({
	...
	factory: (require) => {
				...
				.then(function (d) {
					...
					cb(list.map(function (s) {
						return {
							...
							env: s.env || {},
							cwd: s.cwd || "",
							toolCallTimeoutMs: s.toolCallTimeoutMs || 60000, failOnStartupError: s.failOnStartupError
						};
					}));
				})
				.catch(function () { cb(null); });
		}
		function saveServers(servers, onError, onSuccess) {
			var p = servers.map(function (s) {
				...
				if (s.transport === "stdio") {
					...
					if (s.failOnStartupError !== undefined) out.failOnStartupError = s.failOnStartupError;
				} else {
					out.url = s.url;
			...
			fetch("/remote-mcp-manager", {
				...
				if (!r.ok) {
					return r.text().then(function (t) {
						throw new Error(t ? ("保存失败：" + t) : ("保存失败 (HTTP " + r.status + ")"));
					});
				}
				if (onSuccess) onSuccess();
			}).catch(function (e) {
				if (onError) onError(e.message || "保存失败");
			});
		}
		...
		async function testConnection(server) {
			...
			if (server.path) { var pp = server.path.replace(/^\/+/, "").replace(/\/+$/, ""); if (pp) url.pathname = url.pathname.replace(/\/+$/, "") + "/" + pp; }
			var h = {};
		...
		function ConfirmModal(props) {
			...
			return React.createElement("div", { className: "remoteMcpOverlay", onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); } },
				React.createElement("div", { className: "remoteMcpModal", role: "dialog", "aria-modal": "true" },
					...
					props.error ? React.createElement("div", { className: "remoteMcpFormError", style: { marginBottom: 12 } }, props.error) : null,
					React.createElement("div", { className: "remoteMcpModalActions" },
						...
						React.createElement("button", { className: "remoteMcpSave", onClick: props.onConfirm, disabled: props.saveDisabled }, props.confirmText || "确定")
					)
				)
			);
		}
		...
		function SettingsCard() {
			...
			var _i = React.useState(""), formError = _i[0], setFormError = _i[1];
			var formInitial = React.useRef(null);
			var firstSave = React.useRef(true);
			var _j = React.useState(""), saveError = _j[0], setSaveError = _j[1];

			React.useEffect(function () { loadServers(function (l) { if (l == null) return; setServers(l); setLoaded(true); }); }, []);
			React.useEffect(function () {
				if (!loaded) return;
				if (firstSave.current) { firstSave.current = false; return; }
				saveServers(servers, setSaveError, function () { setSaveError(""); });
			}, [servers, loaded]);
			...
			function buildServerFromForm() {
				...
				if (form.transport === "stdio") {
					...
					e.cwd = form.cwd || "";
					e.toolCallTimeoutMs = parseInt(form.toolCallTimeoutMs, 10) || undefined;
					e.failOnStartupError = form.failOnStartupError;
				} else {
					e.url = form.url;
					e.path = form.path || "";
					e.token = form.token || "";
					e.toolCallTimeoutMs = Math.max(0, parseInt(form.toolCallTimeoutMs, 10)) || 60000;
				}
				return e;
			}
			...
			// 返回 null=成功,字符串=错误信息
			function saveServer() {
				if (!form.serverName) return "请填写 Server 名称";
				if (form.transport === "streamable-http" && !form.url) return "请填写 URL";
				if (form.transport === "stdio" && !form.command) return "请填写命令";
				if (!/^[a-zA-Z0-9_-]+$/.test(form.serverName)) return "Server 名称只允许字母、数字、下划线(_)、连字符(-)";
				// 重名校验:新建时全部比对;编辑时排除自身(保留原名允许)
				if (servers.some(function (s) { return s.serverName === form.serverName && s.serverName !== editing; })) {
					return "Server 名称已存在";
				}
			...
			var formBody = React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 10 } },
				...
				formError ? React.createElement("div", { className: "remoteMcpFormError" }, formError) : null,
				React.createElement("div", { className: "remoteMcpFooter" },
					...
					React.createElement("button", { className: "remoteMcpSave", onClick: function () { var err = saveServer(); if (err) setFormError(err); }, disabled: (form.transport === "streamable-http" && !form.url) || (form.transport === "stdio" && !form.command) || !form.serverName }, editing ? "保存修改" : "保存配置")
				)
			);

			return React.createElement(React.Fragment, null,
				saveError ? React.createElement("div", { className: "remoteMcpFormError", style: { marginBottom: 8 } }, saveError) : null,
				...
				React.createElement(ConfirmModal, {
					...
					confirmText: "保存",
					error: formError,
					saveDisabled: (form.transport === "streamable-http" && !form.url) || (form.transport === "stdio" && !form.command) || !form.serverName,
					onCancel: function () { setConfirmOpen(false); resetForm(); },
					onConfirm: function () { var err = saveServer(); if (err === null) setConfirmOpen(false); else { setConfirmOpen(false); setFormError(err); } }
				})
			);
		}

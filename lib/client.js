	...
	factory: (require) => {
		...
		function SettingsCard() {
			...
			function saveServer() {
				...
				if (form.transport === "streamable-http" && !form.url) return;
			...
			var formBody = React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 10 } },
				...
				React.createElement("div", { className: "remoteMcpFooter" },
					React.createElement("button", { className: "remoteMcpTest", onClick: function () { setForm({ serverName: "", transport: "streamable-http", url: "", token: "", path: "", command: "", args: "", env: "", cwd: "", toolCallTimeoutMs: "" }); setEditing(null); } }, "重置"),
			...
			return React.createElement(Card, { title: "远程 MCP 管理", description: "配置远程 MCP Server，连接后工具自动注册到 DSH" },
				...
				React.createElement(Card, { title: editing ? "编辑配置" : "新建配置", description: editing ? "修改已添加的 MCP Server" : "添加新的 MCP Server 到 DSH" }, formBody)
			);
		}
